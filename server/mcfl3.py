...
class MetaClusterFLServer3(MetaClusterFLServer):
    def __init__(self) -> None:
        super().__init__()

        self.client_map_cluster = {i: 0 for i in range(len(self.clients))}

    def cluster_train(self, selected_clients_cid: List[int]):
    ...
    def spectral_clustering(self, num_clusters: int):
        ...
        new_cluster_clients = [[] for _ in range(num_clusters)]
        for index, label in enumerate(clustering.labels_):
            label = int(label)
            self.client_map_cluster[index] = label
            new_cluster_clients[label].append(index)
        ...
        for cluster in self.cluster_clients:
            ...
            for cid in cluster:
                if self.accumulated_inactive <= 10:
                    ...
                    results.append(self.clients[cid].evaluate_without_train(self.clients[cid].per_model))
    ...
    def run(self):
        ...
        for current_round in range(self.num_round):
            ...
            if self.accumulated_inactive <= 10:
                ...
                # 常规联邦学习
                for client in selected_clients:
                    client.train_after_cluster(client.per_model)
